 ...
 */
namespace SF\Mvc;
use SF\Controller\Exception;
use SF\InvalidClassException;
use SF\Controller\Exception\PageNotFoundException;
...
class Router {
	...
	public function dispatch($controller = '', $action = '', $args = array()) {
		...
		if(!method_exists($this->_controllerInstance, $method)){
			throw new PageNotFoundException( sprintf('method %s of class %s is not found in %s:%u', $method, $controllerClass,  \SF\Event\Error\Handler::getRelativePath(__FILE__)  ,__LINE__));
			exit(-1);
		}
		$this->_controllerInstance->before();
		try{
			call_user_func_array(array($this->_controllerInstance, $method), $args);
		}catch (\SF\Event\Exception\Exception $ex){
